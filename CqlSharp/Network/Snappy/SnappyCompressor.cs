...
namespace CqlSharp.Network.Snappy
{
    public class SnappyCompressor
    {
        ...
        internal int CompressInternal(byte[] uncompressed, int uncompressedOffset, int uncompressedLength, byte[] compressed, int compressedOffset)
        {
            ...
            for (int read = 0; read < uncompressedLength; read += BlockSize)
            {
        ...
        internal int CompressFragment(byte[] input, int inputOffset, int inputSize, byte[] output, int outputIndex, short[] hashTable)
        {
            ...
            {
                int ipLimit = inputOffset + inputSize - InputMarginBytes;
                ...
                for (uint nextHash = Hash(currentIndexBytes, shift); ; )
                {
                    ...
                    do
                    {
                        ...
                        int matched = 4 + _findMatchLength(input, candidate + 4, inputIndex + 4, inputEnd);
                        inputIndex += matched;
        ...
        private int EmitCopyLessThan64(byte[] output, int outputIndex, int offset, int length)
        {
            ...
            if ((length < 12) && (offset < 2048)) {
                ...
                output[outputIndex++] = (byte) (CqlSharp.Network.Snappy.Snappy.Copy1ByteOffset | ((lenMinus4) << 2) | ((offset >> 8) << 5));
                output[outputIndex++] = (byte) (offset);
            }
            else {
                output[outputIndex++] = (byte) (CqlSharp.Network.Snappy.Snappy.Copy2ByteOffset | ((length - 1) << 2));
                output[outputIndex++] = (byte) (offset);
        ...
        internal int EmitLiteralTag(byte[] output, int outputIndex, int size)
        {
            if (size < 60)
            {
                output[outputIndex++] = (byte)(CqlSharp.Network.Snappy.Snappy.Literal | (size << 2));
            }
            else
            {
                ...
                output[baseIndex] = (byte)(CqlSharp.Network.Snappy.Snappy.Literal | ((59 + count) << 2));
            }
            return outputIndex;
        }

...
namespace CqlSharp.Network.Snappy
{
    public class SnappyDecompressor
    {
        ...
        public void Decompress(byte[] compressed, int compressedSize, out byte[] uncompressed, out int uncompressedSize)
        {
            var sizeHeader = ReadUncompressedLength(compressed, 0);

            uncompressedSize = sizeHeader[0];
            uncompressed = uncompressedSize < MemoryPool.BufferSize ? MemoryPool.Instance.Take() : new byte[uncompressedSize];

            Decompress(compressed, sizeHeader[1], compressedSize - sizeHeader[1], uncompressed, 0, uncompressedSize);
        }

        private int Decompress(byte[] input, int inputOffset, int inputSize, byte[] output, int outputOffset, int outputLimit)
        {
            ...
            while (ipIndex < ipLimit - 5)
            {
                ...
                if ((opCode & 0x3) == CqlSharp.Network.Snappy.Snappy.Literal)
                {
